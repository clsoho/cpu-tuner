...
export default function FivrPanel() {
  ...
  const handleApply = async (target: string, offset: number | null) => {
    ...
    try {
      await api.setFivrOffset(target, offset)
    } catch {}
  }
    ...
    <div>
      <div className="panel" style={{marginBottom:4}}>
        <div className="panel-head">
          <span className="panel-title">FIVR — Voltage Control</span>
          <span className="text-xs text-muted">Full Integrated Voltage Regulator</span>
        </div>
        <div className="panel-body">
          <div className="fivr-layout">
            {/* Left: Offset sliders */}
            <div className="fivr-section">
              <div className="section-title">Offset (mV)</div>
              <div className="fivr-slider-group">
                <OffsetSlider label="Core" value={coreOffset} onChange={v => { setCoreOffset(v); handleApply('core', v) }} />
                <OffsetSlider label="Cache" value={cacheOffset} onChange={v => { setCacheOffset(v); handleApply('cache', v) }} />
                <OffsetSlider label="GPU" value={gpuOffset} onChange={v => { setGpuOffset(v); handleApply('gpu', v) }} />
                <OffsetSlider label="System Agent" value={saOffset} onChange={v => { setSaOffset(v); handleApply('system_agent', v) }} />
              </div>
              ...
              <ul style={{margin:0, padding:'0 0 0 12px', fontSize:10, color:'var(--text-muted)'}}>
                <li>Undervolting has been locked on many 10th-gen+ CPUs</li>
                <li>Start with -50mV and test stability</li>
                <li>Excessive undervolt may cause BSOD</li>
              </ul>
            </div>

            {/* Right: Read-only monitoring */}
            <div className="fivr-section">
              <div className="section-title">Current Voltage</div>
              <div className="fivr-readonly">
                <div className="fivr-ro-item"><span className="fivr-ro-label">Core</span><span className="fivr-ro-val">{coreVolt != null ? `${coreVolt} mV` : '—'}</span></div>
                <div className="fivr-ro-item"><span className="fivr-ro-label">Cache</span><span className="fivr-ro-val">{cacheVolt != null ? `${cacheVolt} mV` : '—'}</span></div>
              </div>
              <div className="ts-sep" />
